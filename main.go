...
var (
	version              = "1.0.0-dev" //don't modify it, Jenkins will take care
	git_revision         = "1.0.0"  //don't modify it, Jenkins will take care
	...
	Tools        tools.Tools
	indexURL     = flag.String("indexURL", "https://downloads.molmc.com/packages/package_staging_index.json", "The address from where to download the index json containing the location of upload tools")
)
...
func main() {
	...
	if *hibernate == false {
		...
		go func() {
			...
			// Instantiate Tools
			usr, _ := user.Current()
			directory := filepath.Join(usr.HomeDir, ".molmc-create")
			Tools = tools.Tools{
				Directory: directory,
				IndexURL:  *indexURL,
				Logger:    log.New(),
			...
			if *origins == "" {
				...
				_, err = f.WriteString("\norigins = http://webide.molmc.com:8080\n")
				if err != nil {
					panic(err)
				}
				f.Close()
            ...
            extraOriginStr := "https://api.intorobot.com, http://api.intorobot.com, http://www.intorobot.com, https://www.intorbot.com, http://112.124.117.64, http://112.124.117.64:8088, http://localhost"
